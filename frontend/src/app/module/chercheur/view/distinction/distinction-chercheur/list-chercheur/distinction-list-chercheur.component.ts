...
@Component({
  selector: 'app-distinction-list-chercheur',
  templateUrl: './distinction-list-chercheur.component.html',
  styleUrls: ['./distinction-list-chercheur.component.css']
})
export class DistinctionListChercheurComponent implements OnInit {
   // declarations
...
navigateToDistinctionCreate(){
this.router.navigate(['/app/chercheur/distinction/create']);
}
         ...
         if(isPermistted){
          this.distinctionService.findByIdWithAssociatedList(distinction).subscribe(res => {
           this.selectedDistinction = res;
            ...
            this.navigateToDistinctionCreate() ;
          });
        }else{
